...
def replace_section(html, section_id, new_content):
    # Try finding <!-- XX SECTION_ID -->
    pattern = rf"(<!-- \d+ {section_id} -->.*?)(<!-- \d+ |</section>)"
    ...
    res = re.sub(pattern, repl, html, flags=re.DOTALL)
    if res == html:
        # Fallback if no digits
        pattern = rf"(<!-- {section_id} -->.*?)(<!-- \d+ |</section>)"
        res = re.sub(pattern, repl, html, flags=re.DOTALL)
    return res

ABOUT = """    <section id="about" class="min-h-screen flex items-center py-32 px-4 md:px-12 relative z-10">
...
SKILLS = """    <section id="skills" class="py-32 px-4 relative spatial-container overflow-hidden bg-[rgba(255,255,255,0.01)] border-t border-white/5">
...
EXP = """    <section id="experience" class="py-32 px-4 md:px-12">
...
PROJECTS = """    <section id="projects" class="py-32 px-4 md:px-12 bg-[rgba(255,255,255,0.01)] border-t border-white/5">
...
ACHIEVEMENTS = """    <section id="achievements" class="py-32 px-4 md:px-12 border-t border-white/5">
        <div class="max-w-5xl mx-auto">
...
EDUCATION = """    <section id="education" class="py-32 px-4 md:px-12 border-t border-white/5">
...
CREDENTIALS = """    <section id="credentials" class="py-32 px-4 md:px-12 bg-[rgba(255,255,255,0.01)] border-t border-white/5">
        <div class="max-w-7xl mx-auto">
...
WORKSHOPS = """    <section id="workshops" class="py-32 px-4 md:px-12 border-t border-white/5">
...
content = replace_section(content, "ABOUT", ABOUT)
content = replace_section(content, "SKILLS", SKILLS)
content = replace_section(content, "EXPERIENCE", EXP)
content = replace_section(content, "PROJECTS", PROJECTS)
content = replace_section(content, "ACHIEVEMENTS", ACHIEVEMENTS)

# Manually replace the rest based on known structure
content = re.sub(r'<!-- 18 CERTIFICATIONS -->.*?</section>', f'<!-- 18 CERTIFICATIONS -->\n{CREDENTIALS}', content, flags=re.DOTALL)
content = re.sub(r'<!-- 19 WORKSHOPS & TRAINING -->.*?</section>', f'<!-- 19 WORKSHOPS & TRAINING -->\n{WORKSHOPS}', content, flags=re.DOTALL)
content = re.sub(r'<!-- 20 BEYOND TECHNOLOGY -->.*?</section>', '', content, flags=re.DOTALL)
content = re.sub(r'<!-- 21 & 22 EDUCATION & EXPLORING -->.*?</section>', f'<!-- 21 EDUCATION -->\n{EDUCATION}', content, flags=re.DOTALL)
# Also fix education if it didn't grab the second div properly
content = re.sub(r'<!-- 21 EDUCATION -->\n.*?<section id="education".*?</section>', f'<!-- 21 EDUCATION -->\n{EDUCATION}', content, flags=re.DOTALL)

# Fix Hero texts
content = re.sub(
    r'<p class="text-meta mt-8 mb-4 reveal-up".*?</p>', 
    '<p class="text-meta mt-8 mb-4 reveal-up" style="transition-delay: 0.2s;">MCA STUDENT · TECH ENTHUSIAST</p>', 
    content, flags=re.DOTALL
)
content = re.sub(
    r'<p class="text-xl md:text-2xl font-light text-muted italic reveal-up".*?</p>', 
    '<p class="text-xl md:text-2xl font-light text-muted italic reveal-up" style="transition-delay: 0.3s;">"Motivated MCA student with interests in User Experience Design, Artificial Intelligence and Software Development."</p>', 
    content, flags=re.DOTALL
)
